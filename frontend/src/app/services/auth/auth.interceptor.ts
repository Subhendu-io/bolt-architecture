...
import { switchMap, catchError, take, finalize, filter, map } from 'rxjs/operators';
    ...
    if (this.isRefreshingToken) {
      ...
      return this.auth.refreshToken().pipe(switchMap((response: any) => {
        if (response && response['success'] && response['_jwt'] !== undefined) {
          ...
          this.auth.currentUser = this.globalService.getJwtUser(this.auth.tokenData['access_token']);
          localStorage.setItem('_cu', this.globalService.getEncrypted(this.auth.currentUser));

          this.tokenSubject.next(this.auth.tokenData['access_token']);
          return next.handle(
            this.addTokenToRequest(request, this.auth.tokenData['access_token'])
          );
        }
      }),
        catchError(err => {
